...
//          }
 ...
 def Kubernetes_Deployment (PASSWORD, IPADDRESS, APPNAME, ENVIRONMENT, HOSTPORT, CONTAINERPORT, IMAGE_REGISTRY, REPO_NAME, GIT_COMMIT) {
      sh """
       sshpass -p '${PASSWORD}' -v ssh -o StrictHostKeyChecking=no devops@${IPADDRESS} gcloud auth activate-service-account --key-file=/home/devops/gcp/key.json && sudo yum install google-cloud-sdk-gke-gcloud-auth-plugin -y && sudo yum install kubectl -y
       ...
       sshpass -p '${PASSWORD}' -v ssh -o StrictHostKeyChecking=no devops@${IPADDRESS} "sed -i -e 's|IMAGE|${IMAGE_REGISTRY}/${REPO_NAME}/${env.appName}:${GIT_COMMIT}|g' -e 's|APPNAME|${APPNAME}-${ENVIRONMENT}|g' -e 's|PORT|${CONTAINERPORT}|g' /home/devops/k8s/deployment.yml"
